...
#define DECLARE_C_FUNCTION(FUNC_NAME) \
  ScriptFunc(#FUNC_NAME, (lua_CFunction)C_##FUNC_NAME);
...
extern VOID ScriptExit();

extern BOOL ScriptLoad(const CHAR* file_name);
extern BOOL ScriptExec(const CHAR* name, const CHAR* string);
extern VOID ScriptFunc(const CHAR* name, lua_CFunction func);
extern BOOL ScriptCall(const CHAR* name, const U8* data, U16 size);

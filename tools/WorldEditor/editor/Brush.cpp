...
VOID BrushCircle::Flush(const Map* map, const Vector3& pt)
{
	...
	for(U32 j = clip.top; j < clip.bottom; j++)
	{
		...
		{
			EChunkPtr chunk =  dynamic_ptr_cast<EChunk>(map->mChunks[i+j*map->mWidth]);
...
BrushLozenge::BrushLozenge(const Map* map) : Brush(map)
{
	...
	if(mLinePtr)
	{		
		mLinePtr->add(Vector3(TILE_STRIDE,0.0f,0.0f));
		mLinePtr->add(Vector3(0.0f,TILE_STRIDE*0.5f,0.0f));
		mLinePtr->add(Vector3(-TILE_STRIDE,0.0f,0.0f));
		mLinePtr->add(Vector3(0.0f,-TILE_STRIDE*0.5f,0.0f));

...
//////////////////////////////////////////////////////////////////////////	
	...
	====================
	*/
	VOID SkeletalMesh::Load(const GData* data)
	{
		GUARD(SkeletalMesh::load);
		...
		CHECK(Str(mesh_config_ptr->GetValue()) == "skeletal_mesh");
		...
		{			
			...
			if( Str(mesh_child_config_ptr->GetValue()) == "primitive" )
			{
				...
				PRIMITIVE primitive;				
				primitive.primitive = GNEW(Primitive); CHECK(primitive.primitive);				

				// get the primitive type
				Str type = primitive_config_ptr->GetAttribute("type");
				if(type=="PT_POINTS") primitive.primitive->SetType(Primitive::PT_POINTS);
				else if(type=="PT_LINES") primitive.primitive->SetType(Primitive::PT_LINES);
				else if(type=="PT_LINE_LOOP") primitive.primitive->SetType(Primitive::PT_LINE_LOOP);
				else if(type=="PT_LINE_STRIP") primitive.primitive->SetType(Primitive::PT_LINE_STRIP);
				else if(type=="PT_TRIANGLES") primitive.primitive->SetType(Primitive::PT_TRIANGLES);
				else if(type=="PT_TRIANGLE_STRIP") primitive.primitive->SetType(Primitive::PT_TRIANGLE_STRIP);
				else if(type=="PT_TRIANGLE_FAN") primitive.primitive->SetType(Primitive::PT_TRIANGLE_FAN);
				else if(type=="PT_QUADS") primitive.primitive->SetType(Primitive::PT_QUADS);
				else if(type=="PT_QUAD_STRIP") primitive.primitive->SetType(Primitive::PT_QUAD_STRIP);
				else if(type=="PT_POLYGON") primitive.primitive->SetType(Primitive::PT_POLYGON);
				...
				{
					// parse the bone
					if( Str(primitive_child_config_ptr->GetValue()) == "bone" )
					{
						// the new bone
						BONE bone;						

						// set the bone name
						bone.name = primitive_child_config_ptr->GetAttribute("name"); CHECK(bone.name != "");

						// set the node local transform
						std::vector<Str>tokens = GTokenize(primitive_child_config_ptr->GetText());
						CHECK(tokens.size() == 16);
						Matrix m; for(U32 k = 0; k < 16; k++) m.ptr()[k] = (F32)::atof(tokens[k].c_str());
						bone.base = m;

						// set the source is NULL
						bone.source = NULL;

						// add the bone to the table
						primitive.bones.push_back(bone);
					}
					else if( Str(primitive_child_config_ptr->GetValue()) == "vertex" )
					{
						...
						primitive.primitive->SetVertexBuffer(vb_ptr);
					}
					// parse the index
					else if( Str(primitive_child_config_ptr->GetValue()) == "index" )
					{
						...
						primitive.primitive->SetIndexBuffer(ib_ptr);
					}
					// parse the shader
					else if( Str(primitive_child_config_ptr->GetValue()) == "shader" )
					{
						...
						primitive.primitive->SetShader(dynamic_cast<Shader*>(shader_key_ptr->Ptr()), pass_name.c_str());
						...
						for( U32 k = 0; constant_config_ptr = shader_config_ptr->GetChild(k); k++ )
						{
							...
							primitive.primitive->SetConstant(constant_name.c_str(), constant_ptr.Ptr());
						}
					}
					...
					else if( Str(primitive_child_config_ptr->GetValue()) == "bounding_box" )
					{
						...
						primitive.primitive->SetBox(box);
					}
					else
					{
				...
				primitive.transforms.resize(primitive.bones.size()*3);
			...
			{
				GAssert( VA("The skeletal mesh`s keyword(%s) is unknown!", mesh_child_config_ptr->GetValue()) );
			}
		}
	...
	====================
	*/
	VOID SkeletalMesh::Bind(const AnimBone* b)
	{
		GUARD(SkeletalMesh::Bind);
		
		CHECK(b);

		// bind the node to the primitive
		for(U32 i = 0; i < mPrimitives.size(); i++)
		{
			PRIMITIVE& primitive = mPrimitives[i];
			for(U32 j = 0; j < primitive.bones.size(); j++)
			{
				BONE& bone = primitive.bones[j];
				if(bone.name == b->name()) bone.source = (AnimBone*)b;
			}
		}

		// bind the children
		const AnimBone*child = NULL; for(U32 i = 0; (child=b->child(i)) != NULL; i++) Bind(child);
	...
	====================
	*/
	VOID SkeletalMesh::Draw(Graph *gc)
	{
		GUARD(SkeletalMesh::Draw);
		...
		{
			PRIMITIVE& primitive = mPrimitives[i];
			const BoundingBox& box = primitive.primitive->GetBox();
			if(!frustum.boxInFrustum(box.min(),box.max()))continue;
			primitive.primitive->GetConstant("gVP")->SetMatrix(gc->vp());
			primitive.primitive->GetConstant("gWorld")->SetMatrix(gc->world());
			for(U32 j = 0; j < primitive.bones.size(); j++)
			{
				BONE& bone = primitive.bones[j];
				Matrix transform;
				if(bone.source) transform.transpose(bone.base * bone.source->world());
				else transform.transpose(bone.base);
				primitive.transforms[j*3+0] = *reinterpret_cast<Vector4*>(&transform.ptr()[0]);
				primitive.transforms[j*3+1] = *reinterpret_cast<Vector4*>(&transform.ptr()[4]);
				primitive.transforms[j*3+2] = *reinterpret_cast<Vector4*>(&transform.ptr()[8]);
			}
			primitive.primitive->GetConstant("gBones[0]")->SetVectorArray(&primitive.transforms[0],primitive.transforms.size());
			gc->draw(primitive.primitive.Ptr());
		}
